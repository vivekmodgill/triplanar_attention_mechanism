...
class TriplanarAttentionLayer(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, kernel_size: int, stride: int = 1, padding: int = 0, output_padding: int = 0, bias: bool = True, dilation: int = 1, padding_mode: str = 'zeros', tr_flag: bool = False) -> None:
        ...
        self.embeddings = nn.ModuleList([
            nn.Sequential(_module({k: (1, v, v) for k, v in _kwargs.items()}), _module({k: (v, 1, 1) for k, v in _kwargs.items()})), # XY-embedding transform.
            nn.Sequential(_module({k: (v, 1, v) for k, v in _kwargs.items()}), _module({k: (1, v, 1) for k, v in _kwargs.items()})), # XZ-embedding transform.
            nn.Sequential(_module({k: (v, v, 1) for k, v in _kwargs.items()}), _module({k: (1, 1, v) for k, v in _kwargs.items()}))  # YZ-embedding transform.
        ])
